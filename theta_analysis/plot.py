...
# TODO: refactor and break into multiple code blocks
#       Split into grabbing means and one for plotting, add logic for ext theta
...
def odor_B_decoding_ext(odor_B_preds, name, use_median=False):
    odor_B_prev = odor_B_preds[:, :, 0]
    odor_B_current = odor_B_preds[:, :, 1]
    odor_B_future = odor_B_preds[:, :, 2]
    ...
    max_cycle = 1800
    x = np.linspace(0, max_cycle, 200) # changed third demsion from 37 to 200, look into this being dynamic
    plt.plot(x, odor_B_prev_central[0, :], color='deepskyblue', alpha=1, label='P(A)')
    plt.plot(x, odor_B_prev_central[1, :], '--', color='deepskyblue', alpha=0.5)
    plt.plot(x, odor_B_prev_central[2, :], '--', color='deepskyblue', alpha=0.5)
    ...
    #plt.plot([240, 240], [0, 1], '--', color='gray')
    #plt.ylim(0.1, 0.6)
    plt.xlim(0, max_cycle) # changed 
    plt.ylabel('Decoded Probability')
    plt.xlabel('Theta Phase')
    plt.title('{} Odor B'.format(name))
    plt.legend()

def odor_C_decoding_ext(odor_C_preds, name, use_median=False):
    odor_C_prev = odor_C_preds[:, :, 1]
    odor_C_current = odor_C_preds[:, :, 2]
    odor_C_future = odor_C_preds[:, :, 3]
...
def rolling_decoding_ext(odor_B_preds, odor_C_preds, name, use_median=False):
    odor_B_prev = odor_B_preds[:, :, 0]
    odor_B_current = odor_B_preds[:, :, 1]
    odor_B_future = odor_B_preds[:, :, 2]
