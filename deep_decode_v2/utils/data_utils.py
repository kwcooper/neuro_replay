...
def filter_trials(trial_info):
    # Select correct, insequence trials
    # exclude odor E
...
def fetch_tet(rat_name): 
    ...
    if rat_name == 'superchris':
        ...
        raise NameError('Name not found. Please check what is being passed, dammit.')
    return tetrode_ids, tetrode_units
...
def prepare_data(rat_name):
    ...
    tetrode_ids, tetrode_units = fetch_tet(rat_name)
...
def stack_data(all_data, window_size, stride):
    ...
    #print('stak_data allData shape: ', all_data.shape) #DEBUG
    n_window = int((n_time - window_size) / stride) # converted to int...
    #print('stak_data n_window:', n_window) #DEBUG
    #print('\nSTACKED DATA', n_trial, n_window, n_neuron, window_size) #DEBUG
